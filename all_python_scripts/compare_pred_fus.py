...
Fusion Enthalpy
Data source:
    (1) 'predicted-fus_rf.csv' ===>> prediction of fusion enthalpy using random forest
    (2) 'predicted-fus_xgb.csv' ===>> prediction of fusion enthalpy using xgboost
    (2) 'predicted-fus_mlp.csv' ===>> prediction of fusion enthalpy using seq-mlp
...
### Load the dataset
dataset = pd.read_csv('predicted_fus.csv', delimiter=',', keep_default_na=False, na_values=[''])
...
ax.set_ylim(min(dataset['fusH']) - 0.3 * interval, max(dataset['fusH']) + 0.3 * interval)

### Set font properties
sizes = [fontsize, 0.8 * fontsize]
...
ax.set_ylabel(r'Predicted $\Delta_{fus}H$ (kJ/mol)', labelpad=10, fontproperties=fonts[0])

### Set ticker position
ticker_arg = [5, 10, 5, 10]
...
#            loc='best', fontsize='14', handletextpad=0.5)

fig.savefig('fig_1h.jpg', dpi=500, bbox_inches='tight')
plt.show()
